...
Site/workbook visibility for a submitter is gated by `WorkbookSiteSubmitter` — but only if the user has *any* such assignment at all. Users with zero `WorkbookSiteSubmitter` rows bypass the gate entirely and fall back to raw AccessMatrix site permissions. This is a deliberate legacy/global-user fallback, not a bug — it means the gating rule is conditional, not absolute.
...
**Two approval-path editors exist by design**, aimed at different users: the standalone Approval Path Builder (WFLWBLD, below) for power users who need multi-level/`SEQUENTIAL` chains, and a simplified per-workbook chain editor (`api_save_site_chain` in `WKBK/views.py`) for regular admins who just need a quick single-level chain. This split is intentional and is staying — see [Consistency Guidelines](#consistency-guidelines) for the "two UIs, one validated backend" rule that's supposed to govern it.

**The gap:** the workbook's simplified editor currently does **not** call into WFLWBLD's validated writer (`save_workflow_draft_levels`). It writes directly to the same `WorkflowLevel`/`WorkflowLevelApprover` tables with materially weaker rules of its own: it hardcodes `approval_mode="ANY_ONE"` (`SEQUENTIAL` is unreachable through this screen), never requires at least one approver per level, and never checks a submitted user is active before inserting them as an approver. A saved-but-invalid chain is only caught at *publish* time, by the shared `publish_workflow_version` check — so the exposure window is real but bounded (you can save garbage, you can't publish it). Pointing this editor at the shared validated writer instead of its own logic is planned but not yet done — see [Known Gaps](#known-gaps).
...
- **`WorkbookSiteSubmitter` gates submitter workbook visibility** for any user who has at least one such assignment. Users with zero assignments intentionally fall back to raw AccessMatrix site permissions (a legacy/global-user path) — the gate is conditional, not absolute.
...
4. **Two UIs are allowed to share one validated backend — they must not fork the validation logic.** When building a simplified UI for a complex configuration screen (e.g. WKBK's per-workbook chain editor vs. the standalone WFLWBLD Approval Path Builder), the simplified UI should call the same validated service functions as the power-user UI, not re-implement its own weaker rules. WKBK's chain editor doesn't yet follow this rule — see [Known Gaps](#known-gaps).
...
- **RPTBLD's report-scoping permission check hand-rolls its own `AccessMatrix` query** and is missing the `entity_type == "all"` wildcard, silently under-serving admins with blanket permissions. (NOTIFY had the identical bug; NOTIFY's fix is in progress but RPTBLD's is not yet addressed.)
- **NOTIFY's recipient resolution hand-rolls its own `AccessMatrix` query** with the same missing-wildcard bug as RPTBLD. Fix planned: call `has_permission()` / `get_user_permissions()` directly.
- **`FormSection` is not versioned.** Editing sections in a draft mutates what's live for the currently published form version too. Fix planned: version sections the same way `FieldVersion` is versioned, so draft edits are isolated until publish.
- **`get_eligible_level_approvers` filters only `is_deleted`, not `is_active`**, inconsistent with `publish_workflow_version`'s active-approver check at publish time. A submission can silently stall at a level whose only approver was later deactivated. Fix planned: filter `is_active` here too.
- **WKBK's simplified chain editor doesn't call WFLWBLD's validated writer.** It hardcodes `approval_mode="ANY_ONE"`, never requires ≥1 approver per level, and never checks a submitted user is active. Bounded by the fact that `publish_workflow_version` still catches invalid chains at publish time — but the editor itself should be pointed at the shared validated logic (see [Consistency Guidelines](#consistency-guidelines)).
