...
def get_approver_queue(user_id):
    ...
    """
    # 1. Fetch all submissions currently in "Under Review" status
    subs = Submission.query.filter(
        Submission.status.in_(("Under Review", "Submitted", "Resubmitted")),
    ...
    for sub in subs:
        ...
        days_waiting = (datetime.now(timezone.utc) - start_time).days if start_time else 0
